...
- 1 kHz akım PI firmware UART ayarı: **2000000 baud, 8 data bit, parity none,
  1 stop bit, flow control none**
...
3. Baud rate değerini firmware ile eşleştirin; 1 kHz akım PI için varsayılan
   `2000000`'dır.
...
- Bağlantı yeşil, bağlantı kurulma/kapanma süreci sarı, bağlantısız durum kırmızı
  gösterilir.
- Seri bağlantı bölümünün altındaki **Current PI Telemetry** paneli,
  `DATA t=...` satırlarındaki hedef/ölçülen akım, hata, duty, integral katkısı,
  uygulanan gerilim, BEMF, Hall/sector ve etkinlik durumunu gösterir. Paneldeki
  buton hedef/ölçülen akım, hata ve duty/integral için üç canlı grafiği ayrı
  bir pencerede açar.
...
- Encoder grafiği `ENC: ...` satırlarından veya birleşik `DATA ... enc=...`
  alanından güncellenir; bu veriler gelene kadar bekleme durumunda kalır.
- 1 kHz `DATA` akışı 20 ms'lik gruplarla taşınır. Akım PI grafik penceresi tüm
  örnekleri 60 saniyeye kadar korurken ana göstergeler yaklaşık 50 Hz, terminal
  yaklaşık 10 Hz yenilenir; böylece örnekleme hızı korunurken UI kilitlenmez.
