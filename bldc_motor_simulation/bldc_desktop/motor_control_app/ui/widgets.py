...
class EncoderChart(_RollingThreeSignalChart):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        super().__init__(
            ...
            y_label="Encoder count",
            y_ticks=None,
            y_range=None,
            curve_names=("Encoder", None, None),
            parent=parent,
        )

    def append_sample(self, encoder_count: int) -> None:
        """Append an encoder count when the STM protocol exposes one."""

        self._title_label.setText("REAL-TIME ENCODER HISTORY")
        self._append_values(encoder_count, encoder_count, encoder_count)


class CurrentPidChartsDialog(QDialog):
    """Show current tracking, error, and PI duty terms in a separate window."""

    HISTORY_MS = 60_000
    DEFAULT_SCALE_SECONDS = 10.0
    SCALE_OPTIONS = (0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0)
    MAX_SAMPLES = 65_536

    def __init__(self, parent: QWidget | None = None) -> None:
        """Create a non-modal chart window that can keep receiving samples."""

        super().__init__(parent)
        self.setWindowTitle("Current PI Charts")
        self.setModal(False)
        ...
        title = QLabel("CURRENT PI TELEMETRY")
        title.setStyleSheet(
            "color: #f0f6fc; font-size: 18px; font-weight: 800;"
        )
        ...
        self.scale_combo = QComboBox()
        self.scale_combo.setToolTip(
            "Zoom the controller-time window down to 100 ms."
        )
        for seconds in self.SCALE_OPTIONS:
            if seconds < 1.0:
                label = f"{int(round(seconds * 1000.0))} ms"
            else:
                label = f"{seconds:g} s"
            self.scale_combo.addItem(label, float(seconds))
        self.scale_combo.setCurrentIndex(
            self.scale_combo.findData(self._visible_seconds)
        )
        self.scale_combo.currentIndexChanged.connect(self._change_time_scale)

        clear_button = QPushButton("Clear Charts")
        clear_button.clicked.connect(self.clear)
        ...
        self._tracking_plot = self._make_plot(
            "TARGET / MEASURED CURRENT",
            "Current",
            "A",
        )
        self._tracking_plot.addLegend(
            offset=(12, 8), brush="#161b22", pen="#30363d"
        )
        self._target_curve = self._tracking_plot.plot(
            name="Target current", pen=pg.mkPen("#58a6ff", width=2.4)
        )
        self._measured_curve = self._tracking_plot.plot(
            name="Measured current", pen=pg.mkPen("#f85149", width=2.4)
        )
        self._tracking_plot.addLine(y=0, pen=pg.mkPen("#30363d", width=1))

        self._error_plot = self._make_plot("CURRENT ERROR", "Error", "A")
        self._error_curve = self._error_plot.plot(
            pen=pg.mkPen("#f0a020", width=2.3)
        )
        self._error_plot.addLine(
            y=0, pen=pg.mkPen("#8b949e", width=1, style=Qt.PenStyle.DashLine)
        )

        self._duty_plot = self._make_plot(
            "DUTY CYCLE / INTEGRAL CONTRIBUTION", "Duty", "%"
        )
        self._duty_plot.setYRange(-5, 105, padding=0)
        self._duty_plot.addLegend(
            offset=(12, 8), brush="#161b22", pen="#30363d"
        )
        self._duty_curve = self._duty_plot.plot(
            name="Duty cycle", pen=pg.mkPen("#bc8cff", width=2.3)
        )
        self._integral_curve = self._duty_plot.plot(
            name="Integral contribution", pen=pg.mkPen("#3fb950", width=2.2)
        )
        ...
        layout.addWidget(self._tracking_plot, 1)
        layout.addWidget(self._error_plot, 1)
        layout.addWidget(self._duty_plot, 1)
    ...
    @staticmethod
    def _make_plot(title: str, y_label: str, units: str) -> pg.PlotWidget:
        """Create one consistently styled PID plot."""

        plot = pg.PlotWidget()
        plot.setBackground("#0b0f14")
        plot.setTitle(title, color="#9da7b3", size="11pt")
        plot.showGrid(x=True, y=True, alpha=0.2)
        plot.setMouseEnabled(x=False, y=False)
        plot.setDownsampling(auto=True, mode="peak")
        plot.setClipToView(True)
        plot.enableAutoRange(axis="y")
        plot.setXRange(0, CurrentPidChartsDialog.DEFAULT_SCALE_SECONDS)
        plot.setLabel("bottom", "Controller time", units="s")
    ...
    def append_sample(
        ...
        *,
        ...
        error_current_a: float,
        duty_percent: float,
        integral_percent: float,
    ) -> None:
        """Buffer one normalized current-control sample for all three plots."""

        if time_ms < 0:
            return
        if self._samples and time_ms < self._samples[-1][0]:
            self.clear()
    ...
    def clear(self) -> None:
        """Drop buffered PID samples and reset all three plots."""

        self._samples.clear()
        for curve in (
            self._target_curve,
            self._measured_curve,
            self._error_curve,
            self._duty_curve,
            self._integral_curve,
        ):
            curve.clear()
        for plot in (
            self._tracking_plot,
            ...
            plot.setXRange(0, self._visible_seconds, padding=0)

    def _change_time_scale(self) -> None:
        """Update the shared horizontal window selected by the user."""
    ...
    def _redraw(self, *, force: bool = False) -> None:
        """Refresh visible curves at a bounded rate while the dialog is open."""

        if not self._samples or (not force and not self.isVisible()):
            return

        latest_time = self._samples[-1][0]
        cutoff = latest_time - self.HISTORY_MS
        while self._samples and self._samples[0][0] < cutoff:
            self._samples.popleft()

        times = [sample[0] / 1000.0 for sample in self._samples]
        targets = [sample[1] for sample in self._samples]
        measured = [sample[2] for sample in self._samples]
        errors = [sample[3] for sample in self._samples]
        duty = [sample[4] for sample in self._samples]
        integral = [sample[5] for sample in self._samples]

        self._target_curve.setData(times, targets)
        self._measured_curve.setData(times, measured)
        self._error_curve.setData(times, errors)
        self._duty_curve.setData(times, duty)
        self._integral_curve.setData(times, integral)

        latest_seconds = latest_time / 1000.0
        right_edge = max(self._visible_seconds, latest_seconds)
        left_edge = max(0, right_edge - self._visible_seconds)
        for plot in (
            self._tracking_plot,
            self._error_plot,
            self._duty_plot,
        ):
            plot.setXRange(left_edge, right_edge, padding=0)
